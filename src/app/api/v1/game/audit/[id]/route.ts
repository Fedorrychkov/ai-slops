...
import { gameAuditService } from '@lib/services/game-audit.service'
import { NextRequest, NextResponse } from 'next/server'

import { UserRole } from '~/api/user'
import { getServerTFromNextRequestAsync } from '~/lib/i18n/server'

/** Run the AI security audit over the game HTML — admin/editor only (moderation aid). */
const handler = (request: NextRequest, authResult: AuthSuccessResult, context?: RouteHandlerContext) =>
  apiErrorHandlerContainer(request)(async (response: typeof NextResponse) => {
    ...
    const result = await gameAuditService.auditGame(id, authResult.payload.sub, t)
